...
class BlogModel extends ModelDefault{
    ...
    {
        $gallerydata = $this->select_data('sceg_blog',array('status'=>1),'updated DESC');
        return $gallerydata;
    }
    ...
    public function uplaodGalleryDetails($request)
    {
        ...
        $insertbanners = $this->insert_data('sceg_blog',$insertdata);
        if($insertbanners != 0){
    ...
    public function deleteGalleryData($request)
    {
        ...
        $delete = $this->delete_data('sceg_blog', array('id' => $request));
        if($delete != 0){
    ...
    {
        $gallerydata = $this->select_data('sceg_blog',array('id'=>$request),'updated DESC');
        return $gallerydata;
    }
    ...
    public function saveditdetails($request)
    {
        ...
        $insertbanners = $this->update_data('sceg_blog',$insertdata,$conduction);
        if($insertbanners != 0){
